...
public class ActivityLevel3 extends FragmentActivity implements OnTouchListener {
	// STUBVIEW
	...
	// DEVICE....
	RelativeLayout rl;
	// Arraylist for puzzle GETTING IT FROM THE METHOD GETPUZZLE IN
	// JParserInLevel2
	...
	final String KEY_VAL_PREFIX_HINT_3_2_ = "VAL_HINT_3_2_";
	final String KEY_VAL_PREFIX_HINT_3_1_ = "VAL_HINT_3_1_";
	int random;
	int randomEntered;
	int textColor = Color.rgb(228, 174, 107);
	SharedPreferences app_preferences_random, app_preferences_coins,
			app_preferences_trick_cat_level3;
	...
	class parsing extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			...
			// noOfHintLine1.clear();
			// noOfHintLine2.clear();
			app_preferences_random = getSharedPreferences("apps_random_3",
					MODE_PRIVATE);
	...
	protected void onPause() {
		...
		app_preferences_trick_cat_level3 = getSharedPreferences(
				"appsPreferenceTrickHint", MODE_PRIVATE);
		Editor sped = app_preferences_trick_cat_level3.edit();
		for (int i = 0; i < numbersHintLine1.size(); i++) {
			System.out.println("boldit,,,,,,,,////22222");
			sped.putInt(KEY_VAL_PREFIX_HINT_3_1_ + i, numbersHintLine1.get(i));
		}
		sped.putInt("sizeHint3_1", numbersHintLine1.size());
		for (int i = 0; i < numbersHintLine2.size(); i++) {
			System.out.println("boldit,,,,,,,,////22222");
			sped.putInt(KEY_VAL_PREFIX_HINT_3_2_ + i, numbersHintLine2.get(i));
		}
		sped.putInt("sizeHint3_2", numbersHintLine2.size());
		sped.commit();
		numbersHintLine1.clear();
		numbersHintLine2.clear();
	...
	protected void onResume() {
		...
		app_preferences_trick_cat_level3 = getSharedPreferences(
				"appsPreferenceTrickHint", MODE_PRIVATE);
		int size1 = app_preferences_trick_cat_level3.getInt("sizeHint3_1", 0);
		if (size1 > 0) {
			...
			for (int j = 0; j < size1; j++) {
				...
				// + j, 0));
				noOfHintLine1.add(app_preferences_trick_cat_level3.getInt(
						KEY_VAL_PREFIX_HINT_3_1_ + j, 0));
				// System.out.println("boldit///....vinty"+app_preferences_trick_cat.getInt(KEY_VAL_PREFIX_HINT
				// + j, 0));
			}
			numbersHintLine1 = noOfHintLine1;
		}
		int size2 = app_preferences_trick_cat_level3.getInt("sizeHint3_2", 0);
		if (size2 > 0) {
			System.out.println("boldit,,,,,,,,////5555");
			for (int j = 0; j < size2; j++) {
				noOfHintLine2.add(app_preferences_trick_cat_level3.getInt(
						KEY_VAL_PREFIX_HINT_3_2_ + j, 0));
			}
			numbersHintLine2 = noOfHintLine2;
		}

		/*
		 * values = new ArrayList<Integer>(); values.clear();
		 * app_preferences_random = getSharedPreferences("apps_random",
		 * MODE_PRIVATE); randomRescued =
		 * app_preferences_random.getInt("randomentered", 1500); myAList = new
		 * ArrayList<Integer>(); int size =
		 * app_preferences_random.getInt("size", 0); if (size > 0) { for (int j
		 * = 0; j < size; j++) {
		 * myAList.add(app_preferences_random.getInt(KEY_VAL_PREFIX + j, 0)); }
		 * values = myAList; } if (values.contains(randomRescued)) {
		 * randomRescued = 1500; } initialize(); initialRandomPuzzle();
		 */
	}
	...
	private void trickHint(int i) {
		app_preferences_trick_cat_level3 = getSharedPreferences(
				"appsPreferenceTrickHint", MODE_PRIVATE);
		SharedPreferences.Editor editorSharing = app_preferences_trick_cat_level3
				.edit();

		editorSharing.putInt("KEY_HINT_CAT_3_", i);
	...
	private void initialize() {
		...
		b_KB_cat.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View arg0, MotionEvent event) {
				...
				app_preferences_trick_cat_level3 = getSharedPreferences(
						"appsPreferenceTrickHint", MODE_PRIVATE);
				int no = app_preferences_trick_cat_level3.getInt(
						"KEY_HINT_CAT_3_", 1);
	...
	private void InfoThemeAlert(String string, String string2) {
		...
		dialogAlert = new Dialog(ActivityLevel3.this, R.style.CustomDialogTheme);
	...
	private void checker() {
		...
		if (anss1.length() == answer1.length()
			...
			if (line1AnswerWrong || line2AnswerWrong) {
				// Toast.makeText(ActivityLevel3.this, "Wrong Answer",
	...
	private void showBaby() {
		...
		if (audio == 1) {
			mpCoin1 = MediaPlayer.create(ActivityLevel3.this, R.raw.coin_sound);
			mpCoin2 = MediaPlayer.create(ActivityLevel3.this, R.raw.coin_sound);
			mpCoin3 = MediaPlayer.create(ActivityLevel3.this, R.raw.coin_sound);
			mpCoin4 = MediaPlayer.create(ActivityLevel3.this, R.raw.coin_sound);
			mpCoin5 = MediaPlayer.create(ActivityLevel3.this, R.raw.coin_sound);
		}
		...
		bpointSuperpb.setBackgroundResource(R.drawable.points15);
	...
	private void imaging(int i) {
		...
		if (i == 1) {
			...
			iv1.setBackgroundResource(this.getResources()
					.getIdentifier("img_2_2_" + puzzle_id + "_1", "drawable",
							getPackageName()));
